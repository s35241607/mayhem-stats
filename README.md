...
| 分頁 | 看什麼 |
|---|---|
| 儀表板 | 總覽：KPI、每日勝率＋場次、最常用英雄與增幅、什麼時候打——每張卡都連到對應分頁；點每日圖的任一欄就列出那天的每一場 |
| 對局紀錄 | 逐場瀏覽：每一列有增幅、終場裝備、我方四人 vs 對手五人的英雄；點進去看 10 人完整戰報 |
| 英雄 | 勝率排行（全部英雄都列出，可依勝率或場次排）、每隻英雄的場次、勝率、KDA、傷害佔比、參團率；點一隻英雄看它的出裝與增幅分析與每一場 |
...
League 客戶端 ──LCU API──> FastAPI（採集器 + 靜態站台）──> SQLite mayhem.db
   ...
   對外開放時（可選）：                                            │
   通道 ──> 唯讀鏡像 mirror.pyw ─────────────────────────────────┘
            127.0.0.1:5058（Discord 登入、唯讀、共用同一個 DB 與 Cube）
```

平常只有一個行程要顧：`uv run app.py` 會一併把 Cube 拉起來。
要對外開放才多跑一個鏡像。
...
（實際對外開放時不要這樣手動跑，用下面的鏡像——本機那份可以維持原樣。）
...
2. **其他玩家的 Riot ID 換成穩定代號**（`玩家 A1B2C3`）。同場玩家的名稱是做隊友分析的必要資料，
   ...
   **前提是站台要登入**：看得到名字的只有通過 Discord 白名單／群組的人。網址要給不認識的人就別關。
   `mirror.pyw` 目前預設是關的（`MASK_NAMES = False`）。
3. **設了登入方式就要先登入**才看得到任何東西（含前端本身與圖示）。兩種擇一，見下面的鏡像那節：
   Discord 登入（`.public_oauth`）或共用密碼（`MAYHEM_PASSWORD`）。
   session 放在記憶體，重啟要重新登入；密碼連續猜錯 10 次會整站冷卻 5 分鐘。
   登入後的 Cube 查詢也有速率限制（每個 session 30 秒 60 次、全站 30 秒 300 次）——
   最重的一種查詢第一次要跑十秒，不必把「大家都不會亂來」當成防線。

沒設這個變數時行為完全不變。

**驗證這層是必要的**：免費的通道服務都只給你「誰拿到網址誰就能看」的公開網址，
...
- **Cube 那三個埠的防火牆規則是必做的**（上面「擋掉 Cube 的對外連線」那節）。
  ...
  tailnet 上的其他裝置也算「外面」。
- **通道只能指向鏡像那個埠（5058）**，不要指向 4000，也不要指向本機那份 5057——
  5057 是可寫、看得到真名、而且沒有登入的。
...
**除非你自己把站台開出去**：公開模式預設會把其他玩家的名稱換成代號，
關掉遮罩（`MASK_NAMES = False`）等於讓所有通過登入的人看得到那些名字。
那是別人的遊戲帳號，開給「一起打的那群人」看是一回事，貼給不認識的人是另一回事。
